...
//Function for Accepting a requests to trips
public class AcceptRequest {
    public AcceptRequest() {}
    
    public String AcceptRequestById(String request_id, String trip_id) {
        try (Connection connection = SqLiteConnection.connect()) {
            if (connection != null) {
                // SQL Statements to update trip requests and trips
                String sql = "UPDATE trip_requests SET status = 'accepted' WHERE request_id = ?";
                String updateSql = "UPDATE trips SET trip_status = 'found member' WHERE trip_id = ?";
